...
- **Issue 4 (Role Enum Parsing Errors):** Resolved the `Invalid enum value. Expected 'ADMIN' | 'ACCOUNTING' | 'CS'` errors blocking User creation natively on the frontend via rigid Zod schema arrays! Redefined global React Form configurations mapping directly to the new 9-Tier Prisma dataset. Validated `403 Forbidden` API boundaries safely preventing Cross-Department KPI isolation violations using `scripts/verify_roles.sh`.

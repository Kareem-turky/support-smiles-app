...
export class TicketReasonsService {
  ...
  async findAll(activeOnly: boolean = false) {
    const where: Prisma.TicketReasonWhereInput = activeOnly
      ? { is_active: true }
      : {};
  ...
  async create(data: {
    ...
    // Check dupe name
    const existing = await this.prisma.ticketReason.findUnique({
      where: { name: data.name },
    });
    if (existing)
      throw new ConflictException('Reason with this name already exists');
  ...
  async update(
    ...
    if (data.name) {
      ...
      if (existing && existing.id !== id)
        throw new ConflictException('Reason name taken');
    }

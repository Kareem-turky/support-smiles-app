...
import { SubmitDailyDoneDto, ReportIssueDto } from './dto/interactive-gamification.dto';
...
export class GamificationActionsController {
    ...
    submitDailyDone(@Body() dto: SubmitDailyDoneDto, @Request() req) {
        ...
        return this.gamificationService.logEvent(req.user.id, 'DAILY_SUBMISSION', 0, { ...dto, total_units: totalUnits });
    }

    @Post('report-issue')
    reportIssue(@Body() dto: ReportIssueDto, @Request() req) {
        return this.gamificationService.logEvent(req.user.id, 'ISSUE_REPORTED', 0, dto);
    }
}

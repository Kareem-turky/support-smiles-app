...
export class IntegrationsService {
  ...
  async processInboundIssue(clientId: string | undefined, dto: CreateIssueDto) {
    ...
    const priority = dto.priority; // Default to provided
    const reasonId = dto.reason_id;

    if (dto.reason_id) {
      const reason = await this.prisma.ticketReason.findUnique({
        where: { id: dto.reason_id },
      });
      if (reason) {
    ...
    try {
      ...
      if (error.code === 'P2002') {
        ...
        if (raceInbox && raceInbox.ticket_id) {
          return {
            request_id: uuidv4(),
            data: { ticket_id: raceInbox.ticket_id, duplicate: true },
          };
        }
        throw error;
      }
  ...
  private async getInternalClientId(): Promise<string> {
    ...
    let client = await this.prisma.integrationClient.findFirst({

...
export class MessagesService {
  ...
  async create(ticketId: string, message: string, user: User) {
    const constTicket = await this.prisma.ticket.findUnique({
      where: { id: ticketId },
    });
    ...
    if (constTicket.created_by !== user.id)
      recipients.add(constTicket.created_by);
    if (constTicket.assigned_to && constTicket.assigned_to !== user.id)
      recipients.add(constTicket.assigned_to);

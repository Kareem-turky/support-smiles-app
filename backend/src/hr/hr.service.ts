...
export class HRService {
  ...
  private async ensureMonthEditable(year: number, month: number) {
    ...
    if (
      ...
      throw new ForbiddenException(
        `HR records for ${year}-${month} are ${hrMonth.status} and cannot be edited.`,
      );
    }
  }
  ...
  async updateEmployee(id: string, dto: any) {
    ...
    if (dto.department_id !== undefined)
      empUpdate.department_id = dto.department_id;
  ...
  async deleteEmployee(id: string) {
    ...
    if (employee.user_id) {
      ...
      await this.prisma.gamificationPoint.deleteMany({
      ...
      await this.prisma.user
        .delete({ where: { id: employee.user_id } })
        .catch(() => null);
    }
  ...
  async resetEmployeePassword(id: string) {
    ...
    if (!employee.user_id)
      throw new BadRequestException(
        'This employee has no user account to reset.',
      );
  ...
  async reopenMonth(year: number, month: number, user: User) {
    ...
    if (hrMonth.status === HRMonthStatus.LOCKED) {
      throw new ForbiddenException(
        'Cannot reopen a LOCKED month. Unlock it first.',
      );
    }

    if (hrMonth.status !== HRMonthStatus.SUBMITTED) {
      throw new BadRequestException(
        'Only SUBMITTED months can be reopened to DRAFT.',
      );
    }
  ...
  async upsertAttendance(dto: CreateAttendanceDto, user: User) {
    ...
    if (
      ...
      (dto.minutes_late || 0) === 0 &&
      attendance.employee.user_id
    ) {
